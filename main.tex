...
EEG data was collected from an 4x8 endodermal electrode array at a frequency of 1000 Hz (31 channels, one channel malfunctioned, Fig. \ref{fig:rat_brain_schematic}, \ref{fig:rat_scull_and_instrument}). The data recording lasts for ~500 seconds, during which epileptic seizures were evoked using 4-aminopyridine and EEG data was recorded from the electrode array. The spiking voltage of each recorded electrode is used as a signal and is referred to as a channel (Fig. \ref{fig:example_eeg_and_kcsd}). 
...
Kernel Current Source Density (kCSD) method was used on the grid of channels to account for possible electrical interference with the direct measurement \cite{Potworowski2012}. The measured potentials produced by kCSD arise as the linear combination of the trans-membrane currents, which is a more direct and localized quantity to measure the neural activity. Therefore current source density distribution was calculated by the kCSD method and used for the analysis.
...
\begin{figure}[H]
  \includegraphics[width=\linewidth]{figures/sample_eeg_signal_with_kcsd.pdf}
  \caption{Sample data from electrode 1, in both its raw form and then in its form after kCSD transformation. Applying kernel Current Source Density extracts the true relationship of neural activity, and in theory eliminates disturbances that might be caused by signal interference.}
...
Using the preprocessed data, a pairwise analysis of the signals was be carried using CCM. For each pair of regions, two directions of causality were be considered, resulting in a complete, directional graph of 12 nodes. Since there were $12$ regions, $2*\frac{12*11}{2}=132$ unique causality relationships were analyzed. The causality measures were recorded on sliding windows of time segments. The time segment lengths were chosen through a heuristic that focused on attaining multiple causality measures within a second. 
% Once the causality measures were attained for each time segment, a combination of heuristic and and statistical measures were used to analyze the significance of the causality measures (see section \ref{subsec:significant_causality}). 

From the causality measures, a graph was constructed for each time segment (Fig. \ref{fig:sample_graph}). So in essence, our experiment converts the EEG recordings to a series of complete, directed, weighted graphs, with the weights changing with every time point depending on the causality of one brain region (node) to another. We use this causality as a measure of brain communication. From these causality graphs we produce a dataset where each data point is a graph, and each graph contains 132 features which are the weights of the edges of the graph. To reduce the dimensionality of the dataset, we apply principle component analysis (PCA) to a degree that ensures a high level of data integrity, but reduces the complexity of our dataset. We analyze the difference between the effect of PCA on each time step dataset. Using the reduced form of the data, we then cluster the datasets using unsupervised learning and apply the silhouette score metric to measure how well defined our clusters are. The best scoring cluster number is the one we use as the number of the brain states. 
